...
static void move_spd_ctrl(uint8_t fast, uint8_t slow)
{
  ...
  {
    km.move = NORMAL_MODE;
    km.x_spd_limit = 0.8f * CHASSIS_KB_MAX_SPEED_X;
    km.y_spd_limit = 0.8f * CHASSIS_KB_MAX_SPEED_Y;
  }
	if(cover_state)
	{
		km.move = SLOW_MODE;
...
static uint8_t twist_timer = 0; //按键缓冲计时器
static void chassis_operation_fun(uint8_t twist_chassis)
{

  if (twist_chassis  &&!twist_flag)
	{
		twist_flag = 1;
		km.twist_ctrl = !km.twist_ctrl;
	}
	if (twist_flag)
	{
		twist_timer++;
...
uint8_t cover_state = 0;//cover
static uint8_t cover_flag = 0;//cover
static uint8_t cover_timer = 0;//cover
static void kb_cover_ctrl(uint8_t cover)
{
	if (cover && !cover_flag) 
	{
			cover_flag = 1;
...
static void kb_shoot_cmd(uint8_t trible_fir, uint8_t single_fir,uint8_t one_shoot, uint8_t cont_fir)
{	
	if(trible_fir)
	{
		shoot.firemode = TRIPLE_SHOOT;
		//printf("3");
	}
  if (single_fir)
  {
	...
	if(one_shoot)
	{
		if(shoot.firemode != TRIPLE_SHOOT)
		{
			shoot.firemode = SINGLE_SHOOT;
		}
		shoot.shoot_cmd   = 1;
...
static void gimbal_operation_fun(int16_t pit_ref_spd, int16_t yaw_ref_spd,
                                  uint8_t shoot_small_buff, uint8_t shoot_big_buff,  uint8_t track_armor)
{
  km.pit_v = -pit_ref_spd * 0.02f;
  km.yaw_v = -yaw_ref_spd * 0.02f;
  if (shoot_small_buff)
	{
		km.buff_ctrl = 1;
		...
		pc_send_buf[0] = 205;
		pc_send_buf[1] = 0;
		HAL_UART_Transmit(&huart6, pc_send_buf, 2,100); 
		pc_send_buf[0] = PC_MODE_ID_s;
		pc_send_buf[1] = 1;
		HAL_UART_Transmit(&huart6, pc_send_buf, 2,100);
	}
  if (shoot_big_buff)
...
void keyboard_chassis_hook(void)
{
  if (km.kb_enable)
  {
    move_spd_ctrl(FAST_SPD, SLOW_SPD);   //快速慢速
    
    move_direction_ctrl(FORWARD, BACK, LEFT, RIGHT); //移动方向
    
    chassis_operation_fun(TWIST_CTRL); //扭屁股
  }
  else
  {
    km.vx = 0;
...
void keyboard_gimbal_hook(void)
{
  if (km.kb_enable)
  {
    gimbal_operation_fun(rc.mouse.y, rc.mouse.x, SMALL_BUFF_CTRL, BIG_BUFF_CTRL, TRACK_CTRL);
    
    exit_buff_hook(FORWARD, BACK, LEFT, RIGHT);  //前后左右移动触发退出打符
		
		kb_cover_ctrl(KB_COVER); //开关弹仓
  }
  else //键鼠不可用，km归0
  {
    km.pit_v = 0;
...
void keyboard_shoot_hook(void)
{
  ...
  kb_shoot_cmd(KB_TRIBLE_SHOOT, KB_SINGLE_SHOOT, KB_ONE_SHOOT, KB_CONTINUE_SHOOT);
}
