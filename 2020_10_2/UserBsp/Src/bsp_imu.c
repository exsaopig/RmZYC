...
uint8_t mpu_device_init(void)
{
  ...
  mpu_write_reg(MPU6500_GYRO_CONFIG, 3 << 3);
	mpu_write_reg(MPU6500_ACCEL_CONFIG, 2 << 3);
  ist8310_init();
  mpu_offset_cal();
  return 0;
}
...
void mpu_offset_cal(void)
{
  ...
  {
    mpu_read_regs(MPU6500_ACCEL_XOUT_H, mpu_buff, 14);
		ax = mpu_buff[0] << 8 | mpu_buff[1];
    mpu_data.ax_offset += ax;
		ay = mpu_buff[2] << 8 | mpu_buff[3];
    mpu_data.ay_offset += ay;
		az = mpu_buff[4] << 8 | mpu_buff[5];
    mpu_data.az_offset += az;
